#### This file takes 1 argument - gene signature length - and generates 1000 iterations of random performance measures by randomly selecting gene signatures of length indicated in argument
### To generate 1000 random iterations for signatures of lengths 10 - 300, use an iterative batch job by passing arguments starting with 10 up to 300
...
destdir <- paste0(sourcedir, '/subtype/random/')
...
## Get command line argument for gene signature length to generate 1000 random samples
## For example if gene signature = 15, 1000 performance measures are generated by sampling 15 random genes in each iteration
args = commandArgs(trailingOnly=TRUE)
## test if there is at least one argument: if not, return an error
if (length(args)<1) {
  stop("Input file needs gene signature length (enter in value between 10 and 300)", call.=FALSE)
} else {
  sig_len = as.numeric(args[1])
}
...
for (obj in 1:length(subtype_objects)){
  ...
  ## The attributes to be captured for gene signatures of every length
  test_cindex <- vector()
  testindex_sd <- vector()
  train_cindex <- vector()
  ...
  # use of x=1; p=1 and changes to try-catch updated on August 14 2019 to avoid skipping random generated values that errored out
  x=1
  p=1
  while (p <= rep_count){
    
    ## Set seed for reproducibility
    set.seed(x+gene_len+713)
    
    # SETUP PREDICTORS: select sample predictor genes based on target gene set for specific drugs
    random_genes <- sample(1:ncol(model_data_subtype %>% select(-one_of("days", "class"))), size = gene_len)
    random_matrix <- model_data_subtype %>% select(c(random_genes,"days", "class"))
    rownames(random_matrix) <- rownames(model_data_subtype)
    
    ## Print iteration number
    print(paste("Iteration number",x, "length", gene_len))
    ...
    # skip the current iteration if there is an error, else continues
    # this additional step ensures that we get 1000 random samples even if a number of iterations are skipped
    if (class(try_err) == "try-error"){
      x=x+1
      print("error")
      next
    ...
    drug_num <- c(drug_num, paste0(gene_len,x))
  ...
  # write to file
  cox_random_file <- paste0(destdir,"cox_random_results_subtype_",names(subtype_objects)[obj],gene_len,".txt")
  write.table(cox_random_subtype_df, file = cox_random_file, row.names = F)
