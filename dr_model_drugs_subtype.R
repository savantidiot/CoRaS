...
get_index <- function(model_matrix, num_folds){
  ...
  # get mean c.index
  mean_cindex <- mean(unlist(samp_model$c.index), na.rm=T)
  sd_cindex <- sd(unlist(samp_model$c.index), na.rm=T)
...
for (obj in 1:length(subtype_objects)){
  ...
  print(cox_drug_perf_subtype_df)

  # write to file
  dest_folder <- './generateddata/subtype/'
  cox_drug_subtype_file <- paste0(dest_folder,"cox_drug_results_subtype_",names(subtype_objects)[obj],".txt")
